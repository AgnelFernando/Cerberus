...
class RobotState:
    ...
    gait_type: int = 0
    state: str = "invalid"
    us_data: Dict[str, float] = field(default_factory=lambda: {'front': 0.0, 'right': 0.0, 'left': 0.0, 'back': 0.0})
...
class Go1State:
    ...
    def from_dict(cls, data: dict) -> 'Go1State':
        ...
        state.robot = RobotState(
            ...
            us_data=robot_data.get('us_data', [10] * 4)
        )
        return state
